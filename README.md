...
https://www.notion.so/teamsparta/U-Jeans-bf2841dd59954f44a1de1a63079995ff
    
    
  - 협업한 공간입니다. API 명세서, 와이어프레임, 작업 명세서가 있습니다.
...
모듈화를 진행하면서 의존성과 관련된 문제가 발생했습니다.

- **문제 상황:**`notification` 모듈에서 처리해야 할 Kafka 파티션을 `service` 모듈이 가져가는 문제가 발생.
- **원인:**
    - Kafka는 동일한 **컨슈머 그룹 ID**를 사용하는 컨슈머를 하나의 그룹으로 인식하고 파티션을 분배합니다.
    - 하지만 `service` 모듈은 실제로 컨슈머 역할을 하지 않음에도 동일한 컨슈머 그룹 ID를 사용하는 문제가 있었습니다.
    - 모듈 간 상호 의존성으로 인해, 의도하지 않은 컨슈머 그룹 충돌이 발생했습니다.
...
- 첫 번째 접근:
    - `service` 모듈에 컨슈머 그룹 ID를 직접 지정하여 문제를 해결하려고 했으나,
    - **근본적인 해결책이 아니며** 컨슈머 역할을 하지 않는 모듈에 불필요한 설정을 추가하는 방식이라는 판단.
- 두 번째 접근:
    - 근본적인 문제 해결을 위해 **모듈별 의존 관계를 재정비**하자는 결론에 도달.
    - `notification` 모듈과 `service` 모듈 간의 직접적인 의존성을 제거하고, 공통 의존성을 `common` 모듈로 통합.
---
**2. 해결 과정**

1. 기존의 의존 관계를 분석:
    - `notification`과 `service` 모듈이 서로 간접적으로 의존하고 있었음.
    - 이로 인해 컨슈머 그룹 충돌과 같은 예기치 않은 문제가 발생.
2. 의존 관계를 재구성:
...
1. **문제 해결:**
    - `service` 모듈이 더 이상 Kafka의 컨슈머 그룹 ID를 참조하지 않게 되었고,
    - `notification` 모듈의 컨슈머가 처리해야 할 Kafka 파티션을 정상적으로 처리할 수 있었습니다.
2. **추가 이점:**
    - 모듈 간의 의존 관계가 단순화되었으며, 유지보수성과 확장성이 향상되었습니다.
    - 불필요한 의존성을 제거함으로써 코드 구조가 명확해지고, 유사한 문제의 재발 가능성을 줄였습니다.
---
### **[전후 데이터 비교]**
|항목|전 상태|후 상태|
|---|---|---|
|*Kafka 파티션 처리*|<span style="color:red">notification</span> 모듈의 파티션을 <span style="color:red">service</span> 모듈이 가져감|<span style="color:red">notification</span> 모듈이 모든 파티션을 정상적으로 처리|
|*모듈 간 의존성*|상호 의존 관계로 결합도 높음|<span style="color:red">common</span>모듈만 의존하도록 단순화|
